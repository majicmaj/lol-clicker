...
export const useGameState = () => {
  ...
  useEffect(() => {
    const decayInterval = setInterval(() => {
      setGameState(prevState => {
        ...
        const isHighElo = ['DIAMOND', 'MASTER', 'GRANDMASTER', 'CHALLENGER'].includes(prevState.player.rank);
        ...
        const shouldWarn = inactivityTime >= warningThreshold && !prevState.player.inactivityWarning;
        ...
        if (inactivityTime >= inactivityThreshold) {
          ...
          if (newLp === 0) {
            if (rank === 'CHALLENGER') {
              ...
              division = ['4', '3', '2', '1'][(['1', '2', '3', '4'].indexOf(division) + 1) % 4] as '4' | '3' | '2' | '1';
              lp = 75;
            }
          }
  ...
  useEffect(() => {
    const fetchItems = async () => {
      try {
        const response = await fetch('https://ddragon.leagueoflegends.com/cdn/15.2.1/data/en_US/item.json');
        ...
        const processedItems: Item[] = Object.entries(data.data)
          ...
          .map(([id, item]: [string, any]) => ({
            ...
            stats: {
              ...
              attackSpeed: (item.stats.FlatAttackSpeedMod || 0) + (item.stats.PercentAttackSpeedMod || 0),
  ...
  return { gameState, setGameState, items, loading };
};