...
export const GameStats: React.FC<GameStatsProps> = ({ player, inventory }) => {
  ...
  const avgLpGain = lpGains.length > 0
    ? lpGains.reduce((sum, lp) => sum + lp, 0) / lpGains.length
    : 0;

  const avgLpLoss = lpLosses.length > 0
  ...
  const lpChartData = player.lpHistory.map((lp, index) => {
    ...
    const divisionValue = currentDivision ? DIVISION_VALUES[currentDivision] : 0;
  ...
  const formatTooltipValue = (value: number, entry: any) => {
    ...
    return `${data.rank} ${data.division || ''} (${data.change > 0 ? '+' : ''}${data.change} LP)`;
  };

  // Add inactivity warning
  const isHighElo = ['DIAMOND', 'MASTER', 'GRANDMASTER', 'CHALLENGER'].includes(player.rank);
    ...
    <div className="bg-[#091428] p-4 border-2 border-[#C8AA6E] shadow-lg shadow-[#C8AA6E]/20">
      ...
      <div className="grid grid-cols-2 gap-2 mb-3">
        <div className="bg-[#0A1428] p-2 border border-[#0397AB]/30">
          ...
          <div className="text-lg font-bold text-white">+{avgLpGain.toFixed(1)}</div>
        </div>
        <div className="bg-[#0A1428] p-2 border border-[#0397AB]/30">
          <div className="flex items-center gap-1">
          ...
          <div className="text-lg font-bold text-white">-{avgLpLoss.toFixed(1)}</div>
        </div>
      </div>
      ...
      <div className="bg-[#0A1428] p-2 border border-[#0397AB]/30 mb-2">
        ...
        <div className="h-24">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={lpChartData} margin={{ top: 0, right: 0, left: 0, bottom: 0 }}>
              <defs>
